...
void draw() {
  ...
  textSize(width*.02);
  text("d i s p l a y   f o o d   ( s l o w )", width - (width*.06), height *.16);  
  text("s h a k e   s a u c e", width - (width*.06), height *.22);  
  text("c h a n g e   s a u c e", width - (width*.06), height *.28);
    
  textSize(width*.025454);
  text("s a n d w i c h :", width - (width*.06), (height*.4));
  textSize(width*.02);
  ...
  if (command_sel != 0) {
    ...
    if (buffer < 360) {
      if (command_sel == 1) {
        text("  d i s p l a y i n g . . .", width*.06, height*.35);
      } if (command_sel == 3) {
        text("  s h a k i n g . . .", width*.06, height*.35);
      } if (command_sel == 5) {
        text("  c h a n g i n g . . .", width*.06, height*.35);
        
      } if (command_sel == 7 || command_sel == 9 || command_sel == 11) {
        text("  p l a c i n g   i t e m . . .", width*.06, height*.35); 
      } if (command_sel == 13) {
  ...
  if (key == CODED) {
    ...
    if (command_sel == 0) {
      if (keyCode == UP) {
        ...
        } else if (selector_y == height*.49){
          selector_y -= height*.21;
        } else if ((selector_y <= height*.61)&&(selector_y > height*.49)) {
          selector_y -= height*.06;
        } else if ((selector_y == height*.79)||(selector_y == height*.7)) {
          selector_y -= height *.09;
        } 
        if (selector_x == width*.06) {
          selector_y = height*.79;
          selector_x = width*.4636;
        }
      ...
      if (keyCode == DOWN) {
        ...
        if (selector_x != width *.06) {
          if (selector_y < height*.28) {
            selector_y += height*.06;
          } else if (selector_y == height*.28){
            selector_y += height*.21;
          } else if (selector_y < height*.61){
            selector_y += height*.06;
          } else if (selector_y >= height*.61 && selector_y < height*.79) {
            selector_y += height*.09;
          } else if (selector_y == height*.79) {
            selector_y = height * .35;
            selector_x = width*.06;
          }
        }
